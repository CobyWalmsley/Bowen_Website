# -*- coding: utf-8 -*-
"""Elon Backtest - mirror of the Trump home page using Elon results."""
...
st.set_page_config(page_title="Elon Backtest", layout="wide")
st.title("Elon Backtest")

HERE = Path(__file__).resolve().parent.parent
DEFAULT_RESULTS = HERE / "backtest_results_elon.csv"
...
with st.sidebar:
    ...
    upload = st.file_uploader("Backtest results CSV", type=["csv"], key="elon_results_upload")
    if upload is not None:
        df = load_results(upload)
        st.caption(f"Using uploaded file ({upload.name})")
    elif DEFAULT_RESULTS.exists():
        df = load_results(str(DEFAULT_RESULTS))
...
with st.sidebar:
    ...
    if SCORE_COL in df.columns:
        ...
        avail_scores = []
    selected_scores = st.multiselect("Importance score (out of 10)", avail_scores, default=avail_scores, key="elon_scores")
    portfolio_window = st.selectbox("Portfolio holding window", WINDOW_LABELS, index=WINDOW_LABELS.index("1d"), key="elon_port_window")
    drop_zero = st.checkbox("Exclude decision == 0 (ignore) from portfolio", value=True, key="elon_drop_zero")
...
with dcol1:
    dist_window = st.selectbox("Horizon", WINDOW_LABELS, index=WINDOW_LABELS.index("1d"), key="elon_dist_window")
with dcol2:
    dist_norm = st.selectbox("Normalize", ["Counts", "Probability density"], index=1, key="elon_dist_norm")
with dcol3:
    nbins = st.slider("Bins", min_value=10, max_value=80, value=30, step=5, key="elon_nbins")
