# -*- coding: utf-8 -*-
"""Single Elon Tweet - per-row inspector."""
...
DEFAULT_RESULTS = Path(__file__).resolve().parent.parent / "backtest_results_elon.csv"

st.set_page_config(page_title="Single Elon Tweet", layout="wide")
st.title("Single Elon Tweet")
...
with st.sidebar:
    ...
    upload = st.file_uploader("Override with a different CSV (optional)", type=["csv"], key="single_elon_upload")
...
with sel_col2:
    st.write("")
    if st.button("Random row", use_container_width=True, key="elon_random_btn"):
        st.session_state["elon_row_override"] = int(np.random.randint(0, len(df)))
        st.rerun()

if "elon_row_override" in st.session_state:
    row_idx = st.session_state.pop("elon_row_override")
